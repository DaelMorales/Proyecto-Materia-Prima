...
$tipoSubProd = $_GET['tipoSubProd'] ?? null;

if (!$tipoProd || !$tipoSubProd) {
    die("Faltan parámetros tipoProd o tipoSubProd.");
}

$stmt = $conexion->prepare("SELECT idProd, tipo, contenido, precio, tipoSubProd, tipoProd, descrip, correo FROM productos WHERE tipoProd = ? AND tipoSubProd = ?");
if (!$stmt) {
...
<body>
    ...
    <script>
       function mostrarPanelComentario(idProd) { 
        ...
        let correo = getCookie("correoc"); 
        if (comentario && idProd && correo) { 
                ...
                if (this.readyState === XMLHttpRequest.DONE && this.status === 200) { 
                    alert("Comentario agregado exitosamente.");
                     cerrarPanelComentario(); location.reload();
                      ...
                      xhr.send("comentario=" + comentario + "&idProd=" + idProd + "&correo=" + correo); 
                    } else { 
